function K = mehul_experiment_full()
	...
	f_exp = zeros((d+1)*(d^2-1),1);
	dirtyf_exp = zeros((d+1)*(d^2-1),1);
	base = mub_basis(d);
	counter = 0;
	for k=1:d+1
		for i=0:d-1
			for j=0:d-1
				if i ~= d-1 || j ~= d-1
					...
					dirtyf_exp(counter) = dirty_p(i+1,j+1,k);				
				end
			end
		end
	end

	sigma = zeros((d+1)*(d^2-1),(d+1)*(d^2-1));
	for k=1:d+1
		s = (k-1)*(d^2-1)+1;
		e = k*(d^2-1);
		sf_exp = dirtyf_exp(s:e);
...
function objective = hae_mub_full(f_exp,sigma,zeta,d,m)
